...
addEvent("coke3:animacja", true)
addEventHandler("coke3:animacja", getRootElement(), function() 
setPedAnimation ( source, 'GANGS', 'prtial_gngtlkG', -1, false, false )
end)

addEvent("coke3:zanimacja", true) 
addEventHandler("coke3:zanimacja", getRootElement(), function() 
	local exp = 1
...
addEvent("coke3:start", true)
addEventHandler("coke3:start", getRootElement(),
function()
end)

addEvent("daj:coke3", true)
addEventHandler("daj:coke3", root, function()
	local cena = math.random(230, 290)
	if getElementData(source, "user:premium") then
		cena = cena + cena*0.50
	end
