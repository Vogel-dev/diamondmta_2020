...
local coke2_priv2_start = createMarker(-45.755683898926,1403.8934326172,1084.4370117188-0.95, 'cylinder', 1.2, 255, 255, 255)
setElementInterior(coke2_priv2_start, 8)
setElementDimension(coke2_priv2_start, 1073)
setElementData(coke2_priv2_start, 'marker:icon', 'drugs')

local coke2_priv2_miejsca = {
...
bindKey("e", "down", function()
    if not isElementWithinMarker(localPlayer, coke2_priv2_start) then return end
    ...
    if not getElementData(localPlayer, "user:illegal") then
        local coke2_priv2_losuj = math.random(2, #coke2_priv2_miejsca)
        ...
        local coke2_priv2_cel = createMarker(coke2_priv2_miejsca[coke2_priv2_losuj][1], coke2_priv2_miejsca[coke2_priv2_losuj][2], coke2_priv2_miejsca[coke2_priv2_losuj][3]-0.95, "cylinder", 1.0, 255, 255, 255, 90)
        local coke2_priv2_blip = createBlipAttachedTo(coke2_priv2_cel, 41)
        setElementInterior(coke2_priv2_cel, 8)
        setElementDimension(coke2_priv2_cel, 1073)
        setElementData(coke2_priv2_cel, 'marker:icon', 'drugs')

        addEventHandler("onClientMarkerHit", coke2_priv2_cel, function(el, md)
            if not md or getElementType(el) ~= "player" then return end
            if el ~= localPlayer then return end
            ...
            destroyElement(coke2_priv2_blip)
            setElementFrozen(el, true)
            triggerServerEvent ("coke2_priv2:animacja", localPlayer)
			addEventHandler("onClientRender",root,coke2_priv2_pasek) 
			tick = getTickCount()
            setTimer(function()
                setElementFrozen(el, false)
                destroyElement(coke2_priv2_cel)
                triggerServerEvent ("coke2_priv2:zanimacja", localPlayer)
                setPedAnimation(localPlayer, false)
                toggleControl("sprint", true) 
                toggleControl("jump", true)
                setElementData(el, "user:illegal", false)
                removeEventHandler("onClientRender",root,coke2_priv2_pasek)
