...
local coke3_priv1_start = createMarker(-34.986156463623,1408.0369873047,1084.4296875-0.95, "cylinder", 1.3, 255, 255, 255)
setElementInterior(coke3_priv1_start, 8)
setElementDimension(coke3_priv1_start, 1072)
setElementData(coke3_priv1_start, 'marker:icon', 'drugs')

local coke3_priv1_miejsca = {
...
bindKey("e", "down", function()
    if not isElementWithinMarker(localPlayer, coke3_priv1_start) then return end
    ...
    if not getElementData(localPlayer, "user:illegal") then
        local coke3_priv1_losuj = math.random(2, #coke3_priv1_miejsca)
        ...
        local coke3_priv1_cel = createMarker(coke3_priv1_miejsca[coke3_priv1_losuj][1], coke3_priv1_miejsca[coke3_priv1_losuj][2], coke3_priv1_miejsca[coke3_priv1_losuj][3]-0.95, "cylinder", 1.0, 255, 255, 255, 90)
        local coke3_priv1_blip = createBlipAttachedTo(coke3_priv1_cel, 41)
        setElementData(coke3_priv1_cel, 'marker:icon', 'drugs')
        setElementInterior(coke3_priv1_cel, 8)
        setElementDimension(coke3_priv1_cel, 1072)

        addEventHandler("onClientMarkerHit", coke3_priv1_cel, function(el, md)
            if not md or getElementType(el) ~= "player" then return end
            if el ~= localPlayer then return end
            ...
            destroyElement(coke3_priv1_blip)
            setElementFrozen(el, true)
            triggerServerEvent ("coke3_priv1:animacja", localPlayer)
			addEventHandler("onClientRender",root,coke3_priv1_pasek) 
			tick = getTickCount()
            setTimer(function()
                setElementFrozen(el, false)
                destroyElement(coke3_priv1_cel)
                triggerServerEvent ("coke3_priv1:zanimacja", localPlayer)
                setPedAnimation(localPlayer, false)
                toggleControl("sprint", true) 
                toggleControl("jump", true)
                setElementData(el, "user:illegal", false)
                removeEventHandler("onClientRender",root,coke3_priv1_pasek)
                triggerServerEvent("daj:coke3_priv1", localPlayer)
