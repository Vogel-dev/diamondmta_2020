...
PJC.licenses = {
    C = {
        ...
        pos = {2443.4382324219,-1663.9471435547,1016.1433105469-1},

        marker = false,

        veh = {414, 1782.712890625,-1701.3446044922,13.16057491302,0,0,0},
    },
}

for i,v in pairs(PJC.licenses) do
...
PJC.onRender = function()
    local cost = ""
    if(PJC.licenses[PJC.type].cost > 0)then
        cost = "\n\nCena: "..PJC.licenses[PJC.type].cost.." $"
    end
    dxDrawRectangle(0, 0, 1920, 1080, tocolor(15, 15, 15, 200), false)
    dx:dxLibary_createWindow(714/zoom, 307/zoom, 493/zoom, 467/zoom)
    dx:dxLibary_text("Prawo jazdy kat.C", 724/zoom, 317/zoom, 1197/zoom, 373/zoom, tocolor(105, 188, 235, 255), 10, "default", "center", "center", false, false, false, false, false)
    dx:dxLibary_text("Posiadając prawo jazdy kat.C możesz kierować pojazdami osobowymi oraz podjąć się większości prac."..cost, 738/zoom, 485/zoom, 1187/zoom, 569/zoom, tocolor(255, 255, 255, 255), 5, "default", "center", "center", false, true, false, false, false)
    ...
    onClick(748/zoom, 678/zoom, 202/zoom, 51/zoom, function()
        removeEventHandler("onClientRender", root, PJC.onRender)
        showCursor(false)

        for i,v in pairs(PJC.licenses) do
            if(isElementWithinMarker(localPlayer, v.marker))then
                triggerServerEvent("start.pjC", resourceRoot, v, i)
            end
        end
    end)
...
addEventHandler("onClientMarkerHit", resourceRoot, function(hit, dim)
    ...
    for i,v in pairs(PJC.licenses) do
        if(source == v.marker)then
            addEventHandler("onClientRender", root, PJC.onRender)
            showCursor(true)
            PJC.type = i
            return
        end
    ...
    if(source == PJC.marker)then
        destroyElement(PJC.marker)
        destroyElement(PJC.blip)
        
        PJC.point = PJC.point+1

        for i,v in pairs(PJC.licenses) do
            if(i == PJC.type)then
                if(PJC.point == #v.points)then
                    triggerServerEvent("stop.pjC", resourceRoot, i)
                    ...
                    PJC.marker = createMarker(v.points[PJC.point][1], v.points[PJC.point][2], v.points[PJC.point][3], "checkpoint", 2, 105, 188, 235, 255)
                    PJC.blip = createBlipAttachedTo(PJC.marker, 41)
                end
            end
        end
    end
...
addEventHandler("start.pjC", resourceRoot, function(v, type)
    ...
    for i,v in pairs(PJC.licenses) do
        if(i == type)then
            PJC.marker = createMarker(v.points[1][1], v.points[1][2], v.points[1][3], "checkpoint", 2, 105, 188, 235, 255)
            PJC.blip = createBlipAttachedTo(PJC.marker, 41)
        end
    end
end)
...
addEventHandler("stop.pjC", resourceRoot, function(v, type)
    if(PJC.marker and isElement(PJC.marker))then
        destroyElement(PJC.marker)
        destroyElement(PJC.blip)
    end
end)