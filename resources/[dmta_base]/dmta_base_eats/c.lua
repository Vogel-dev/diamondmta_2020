...
function gui()
	exports['dxLibary']:dxLibary_createWindow(718/scale, 343/scale, 489/scale, 318/scale, false)
	...
	for i, v in ipairs(rzeczy) do
		local x,y,w,h = exports['dxLibary']:dxLibary_formatTextPosition(1180/scale, 348/scale, 21/scale, 21/scale)
		if isMouseIn(1180/scale, 348/scale, 21/scale, 21/scale) then
		...
		--exports['dxLibary']:dxLibary_text('Hamburger - 15 USD\nHotDog - 5 USD\nKebab - 10 USD\nWoda - 3 USD\nCola - 4 USD', 747/scale, 373/scale, 1096/scale, 627/scale, tocolor(255, 255, 255, 255), 15, 'default', 'right', 'center', false, false, false, false, false)
...
addEventHandler("onClientClick", root, function(btn, state)
  if btn == "left" and state == "down" then
  	for i,v in ipairs(rzeczy) do
  		local dodatekY = (54*py)*(i-1)
  		if isMouseIn(826*px, 301*py+dodatekY, 100*px, 50*py) and okno == true then
    ...
    if isMouseIn(1180/scale, 348/scale, 21/scale, 21/scale) and okno == true then
